...
def MainExecution():
    try:
        ...
        if not Decision:
            raise ValueError("No decision made by FirstLayerDMM.")
        ...
        # General or Realtime Query
        if (G and R) or R:
            SetAssistantStatus("Searching ...")
            Answer = RealtimeSearchEngine(QueryModifier(Merged_query))
            if not Answer:
                raise ValueError("No answer from RealtimeSearchEngine.")
            ...
            for Queries in Decision:
                if "general" in Queries:
                    ...
                    if not Answer:
                        raise ValueError("No answer from ChatBot.")
        ...
        ShowTextToScreen(f"{Assistantname} : An error occurred, please try again.")
...
def FirstThread():
    while True:
        ...
        if CurrentStatus == "True":
            MainExecution()
            # Keep the microphone on after successful communication
